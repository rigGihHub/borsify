...
def build_inflection_metrics(
    ...
    return {
        ...
        "Marginal YoY föregående kvartal": (_num(margin_series.iloc[1]) - _num(margin_series.iloc[5])) if len(margin_series) >= 6 else np.nan,
        "Marginal QoQ förändring": qoq_margin,
        "FCF YoY senaste kvartal": _yoy(fcf),
        "FCF YoY föregående kvartal": _yoy(fcf.iloc[1:]) if len(fcf) >= 6 else np.nan,
        "Vinst YoY senaste kvartal": _yoy(net_income),
        "Vinst YoY föregående kvartal": _yoy(net_income.iloc[1:]) if len(net_income) >= 6 else np.nan,
