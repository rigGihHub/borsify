...
def render_overview(
    ...
    if daily_shortlist.empty:
        ...
        with st.container(border=True):
            st.caption("1 · BORSIFYS FÖRSTAVAL")
            ...
            st.markdown(f"**När kan det hända?:** {first.get('Decision Brief timing', '❔ Okänd')}")
            st.markdown(f"**Största risk:** {plain_finance_text(first.get('Decision Brief risk', first_risk))}")
            st.caption(f"Hur bra är informationen?  {first.get('Decision Brief confidence', first.get('Analysis Confidence', '—'))}")
        ...
        if len(focus_cases) > 1:
            ...
            for pos in range(1, len(focus_cases)):
                ...
                with st.container(border=True):
                    ...
                    if st.button("Öppna analys och AI", key=f"overview_alt_{ticker}", use_container_width=True):
                        st.session_state["bq_quick_open_ticker"] = ticker
                        st.session_state["bq_quick_open_mode"] = "analysis"
                        st.rerun()
        ...
        with st.expander("Om dagens analys", expanded=False):
            ...
            if isinstance(scan_metrics, dict) and scan_metrics:
                st.caption(
                    ...
                    f"analys/lagring {float(scan_metrics.get('analysis_seconds',0) or 0):.1f} s · "
                    f"totalt {float(scan_metrics.get('total_seconds',elapsed) or elapsed):.1f} s · "
                    f"fundamental cacheträff {float(scan_metrics.get('fundamental_cache_hit_rate',0) or 0):.0%} · "
        ...
        if recs.empty:
            ...
            if summary.get("evaluated", 0):
                ...
                if horizon_options:
                    ...
                    st.markdown("#### Signal Evidence Lab")
                    _sel_h = st.selectbox("Signal-labb · horisont", ["1m","3m","1y"], index=1, key="signal_evidence_horizon")
                    _sel = build_signal_evidence(recs, outs, _sel_h)
                    _sels = lab_summary(_sel)
                    st.caption(_sels["message"])
                    if not _sel.empty:
                        _show=_sel.copy()
                        for _c in ["Median signal","Median control","Median edge","Hit signal","Hit control","Gain10 signal","Loss10 signal"]:
                            _show[_c]=(_show[_c]*100).round(1)
                        st.dataframe(_show, use_container_width=True, hide_index=True)
                        _red=redundancy_pairs(redundancy_matrix(recs))
                        if not _red.empty:
                            st.markdown("##### Möjlig signalredundans")
                            _redshow=_red.copy(); _redshow["Korrelation"]=(_redshow["Korrelation"]*100).round(0)
                            st.dataframe(_redshow,use_container_width=True,hide_index=True)
                    _gov = nominate_signal_actions(_sel, _red if '_red' in locals() else None)
                    if not _gov.empty:
                        st.markdown("##### Signal Kill / Promote Candidates")
                        st.caption(governance_summary(_gov))
                        st.dataframe(_gov, use_container_width=True, hide_index=True)
                    else:
                        st.info(_sels["status"])
    
                    _mh_dq = decision_quality_table(recs, outs, chosen_h)
                    _mh = assess_model_health(_sel, _gov, _red if '_red' in locals() else None, _mh_dq)
                    st.markdown("#### Model Health · evidensmognad")
                    _mh1,_mh2,_mh3,_mh4 = st.columns(4)
                    _mh1.metric("Model Health", f"{_mh['Model Health Score']:.0f}/100")
                    _mh2.metric("Mogna signaler", f"{_mh['Model Health mogna']}/{_mh['Model Health signaler']}")
                    _mh3.metric("Lovande", int(_mh["Model Health lovande"]))
                    _mh4.metric("Varning/retire", int(_mh["Model Health varningar"] + _mh["Model Health retire"]))
                    st.caption(_mh["Model Health förklaring"])

                    _risk_source_rows = build_source_health_rows(st.session_state)
                    _risk_source_summary = summarize_source_health(_risk_source_rows)
                    _risk_register = build_model_risk_register(_mh, _sel, _gov, _red if '_red' in locals() else None, _mh_dq, _risk_source_summary)
                    _risk_summary = summarize_model_risks(_risk_register)
                    st.markdown("#### Model Risk Register")
                    st.caption(_risk_summary["status"] + " · " + _risk_summary["text"])
                    st.dataframe(_risk_register, use_container_width=True, hide_index=True)
                    _risk_roadmap = build_risk_roadmap(_risk_register)
                    _risk_focus = roadmap_focus(_risk_roadmap)
                    st.markdown("#### Risk-to-Roadmap")
                    if not _risk_roadmap.empty:
                        st.info(f"**{_risk_focus['next']}** · {_risk_focus['reason']}")
                        st.dataframe(_risk_roadmap, use_container_width=True, hide_index=True)
                        _execution_plan = build_execution_plan(_risk_roadmap)
                        _next_safe = next_safe_work_item(_execution_plan)
                        st.markdown("##### Säker genomförandeplan")
                        st.caption(f"**{_next_safe['status']}:** {_next_safe['work']} · {_next_safe['done']}")
                        st.dataframe(_execution_plan, use_container_width=True, hide_index=True)
                    else:
                        st.success(_risk_focus["reason"])
                    st.caption("Roadmapen prioriterar utvecklingsarbete från observerade modellrisker. Den ändrar aldrig signaler, vikter eller produktion automatiskt.")
                    if _risk_summary["critical"]:
                        st.error("Kritisk modellrisk finns. Ingen automatisk modelländring görs; riskerna ska granskas manuellt.")
                    elif _risk_summary["high"]:
                        st.warning("Höga modellrisker finns. Prioritera riskåtgärder före ytterligare modellkomplexitet.")

                    st.markdown("#### Kalibrering av Deal Conviction")
                    _dc_horizon = st.selectbox("Conviction-horisont", ["1m","3m","1y"], index=1, key="deal_conviction_cal_horizon")
                    _dc = calibration_by_deal_conviction(recs, outs, _dc_horizon)
                    st.caption(_dc["message"])
                    if _dc["excluded_legacy"]:
                    ...
                    if _dc["table"] is not None and not _dc["table"].empty:
                        ...
                        if _dc["monotonic"] is True:
                            st.success("Hittills stiger medianutfallet monotont mellan tillräckligt stora conviction-grupper. Det är ännu inte ett bevis.")
                        elif _dc["monotonic"] is False:
                            st.warning("Högre Deal Conviction har ännu inte gett monotont bättre medianutfall. Vikterna bör inte förstärkas utan mer evidens.")
                    else:
                        st.info(_dc["status"])

                    st.markdown("#### Decision Quality Calibration")
                    st.caption(
                        "Borsify jämför de frysta beslutslägena mot senare utfall. "
                        "Riskkolumnen är sämsta observerade avkastning under utfallshorisonten – inte full max drawdown."
                    )
                    _dq = decision_quality_table(recs, outs, chosen_h)
                    ...
                    if not _dq.empty:
                        ...
                        if _dq_cmp["status"]=="Confidence-lagret ser lovande ut":
                            st.success(_dq_cmp["message"])
                        elif _dq_cmp["status"]=="Confidence-lagret bör granskas":
                            st.warning(_dq_cmp["message"])
                        else:
                            st.info(_dq_cmp["message"])
                    else:
                        st.info("Decision Support har ännu inte tillräckligt med mogna point-in-time-utfall för kalibrering.")
                    st.caption(
                        f"Minst {MIN_CASES_PER_QUADRANT} mogna case krävs i vardera jämförd grupp innan Borsify drar en jämförande slutsats."
                    )
...
def main() -> None:
    ...
    if last_refresh:
        try:
            ...
            age_minutes = max(0.0, (pd.Timestamp.now(tz="UTC") - last_ts).total_seconds() / 60.0)
            if age_minutes <= 20:
                refresh_status = "Färsk data"
            elif age_minutes <= 120:
                refresh_status = "Delvis färsk"
            else:
                refresh_status = "Gammal data"
            refresh_detail = f"Senast manuellt uppdaterad {last_ts.tz_convert('Europe/Stockholm').strftime('%Y-%m-%d %H:%M')} · {age_minutes:.0f} min sedan."
        except Exception:
            pass
    ...
    if _runtime_health["status"] == "DEGRADED":
        st.warning(
            "Delar av analysen är försvagade: "
            + "; ".join(str(issue.get("impact") or issue.get("component")) for issue in _runtime_health["issues"][:5])
            + ". Grundrankingen kan fortfarande visas, men historik- och lärandefunktionerna ska inte betraktas som kompletta."
        )
        with st.expander("Visa teknisk runtime-status", expanded=False):
            _runtime_table = pd.DataFrame(_runtime_health["issues"]).rename(columns={
                "component": "Komponent", "error_type": "Feltyp", "impact": "Påverkan", "observed_at": "Observerat"
            })
            st.dataframe(_runtime_table, use_container_width=True, hide_index=True)
    ...
    with st.sidebar:
        ...
        with st.expander("Anpassa sökningen", expanded=False):
            ...
            st.caption("Kurser cachas normalt cirka 15 min · bolagsdata upp till 24 h. Använd Uppdatera data högst upp för att tvinga fram en ny hämtning.")
