...
-- v2.41.0: private portfolio / registered purchases.
...
-- v2.45.0: persistent Universe QC state + observation history.
create table if not exists public.universe_qc_state (
  user_id uuid not null references auth.users(id) on delete cascade,
  symbol text not null,
...
alter table public.universe_qc_state enable row level security;
drop policy if exists "universe_qc_state_select_own" on public.universe_qc_state;
create policy "universe_qc_state_select_own" on public.universe_qc_state for select using (auth.uid() = user_id);
drop policy if exists "universe_qc_state_insert_own" on public.universe_qc_state;
create policy "universe_qc_state_insert_own" on public.universe_qc_state for insert with check (auth.uid() = user_id);
drop policy if exists "universe_qc_state_update_own" on public.universe_qc_state;
create policy "universe_qc_state_update_own" on public.universe_qc_state for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

create table if not exists public.universe_qc_events (
  event_id bigint generated by default as identity,
  user_id uuid not null references auth.users(id) on delete cascade,
  symbol text not null,
  outcome text not null,
  reason text not null default '',
  counted_failure boolean not null default false,
  created_at timestamptz not null default now(),
  primary key (user_id, event_id)
);
alter table public.universe_qc_events enable row level security;
drop policy if exists "universe_qc_events_select_own" on public.universe_qc_events;
create policy "universe_qc_events_select_own" on public.universe_qc_events for select using (auth.uid() = user_id);
drop policy if exists "universe_qc_events_insert_own" on public.universe_qc_events;
